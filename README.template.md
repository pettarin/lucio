...
The rendered file opens with a comment saying it was generated,
which `-E` leaves out.
...
The rendered document opens with a comment naming the template it came from, and
a blank line, so that whoever finds the generated file knows what to edit instead:
...
- The output file ends with exactly one newline
  (an empty rendered document produces an empty file, comment included).
- The do-not-edit comment and the blank line below it are the only bytes `lucio`
  adds of its own; `-E` removes even those.
