...
- Replacement rules: a YAML file, named with `-r` / `--rules-file` or found in the
  ...
  the content of an `include` block; a rule is a regular expression or a literal text,
  in `default` or `multiline` mode, respecting or ignoring the case of letters;
  see `res/lucio.rules.yaml` for the syntax
